...
def standardization(data):
    mean = data.mean()
    deviation = data.std()
    standardized_data = (data - mean) / deviation
    return standardized_data

def downsample(data,ori,fre):
    N = ori // fre
    Newdata = []
    for index in range(256):
        Newdata.append(data[index*4,:])
    Newdata = np.array(Newdata)
    return Newdata

def emotion_filter(data,ori_fre,low,high):
    wn1 = 2 * low / ori_fre
    wn2 = 2 * high / ori_fre
...
def get_data(server_port,ip_address,secs):
    ActiveTwo_1 = ActiveTwo( host=ip_address, sfreq=1024, port=int(server_port), nchannels=64, tcpsamples=4)
    rawdata = ActiveTwo_1.read(secs)
    rawdata = standardization(rawdata)
    rawdata = emotion_filter(rawdata,1024,1,46)
    rawdata = downsample(rawdata,1024,256)
    rawdata = rawdata[np.newaxis,np.newaxis,:,:]
    rawdata = np.transpose(rawdata,[0,1,3,2])
    return rawdata
if __name__ == '__main__':
    train_data = get_data(1)
