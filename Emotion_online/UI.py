...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.present.setPixmap(QPixmap("./pic/present.png"))
        self.present.setScaledContents(True)
        self.present.setObjectName("present")

        self.positive = QLabel(self)
        self.positive.setGeometry(QRect(666, 360, 120, 120))
        self.positive.setPixmap(QPixmap("./pic/positive.png"))
        self.positive.setScaledContents(True)
        self.positive.setObjectName("positive")

        self.negative = QLabel(self)
        self.negative.setGeometry(QRect(666, 360, 120, 120))
        self.negative.setPixmap(QPixmap("./pic/negative.png"))
        self.negative.setScaledContents(True)
    ...
    def start_on_click(self):
        ...
        while True:
            if mask == '':
                time.sleep(1)
                print(wait_count, ' mask: ', mask, 'wait')
                wait_count += 1
            elif mask == '0' or mask == '1':
                time.sleep(1)
                if mode == 'train':
                    train(self.serverPort.text(),self.ipAddress.text(),mask)
                elif mode == 'test':
                    result = test(self.serverPort.text(),self.ipAddress.text())
                else:
                    print('Please choose mode !')
                    break
                if result == 0:
                    self.negative.setVisible(False)
                    self.positive.setVisible(True)
                else:
                    self.negative.setVisible(True)
                    self.positive.setVisible(False)
            elif mask == '2':
                print('stop mask: ', mask)
                break
